...
{
	CameraInfo camera;
	auto location_pointer = read<uintptr_t>(Uworld + 0x110); //110
	auto rotation_pointer = read<uintptr_t>(Uworld + 0x120); //120
...
int main(int argc, const char* argv[])
{
	...
	for (auto i = 0; i < INT_MAX; i++) {
		...
		auto gworld = read<uintptr_t>(va_text + offsets::GWORLD);
		auto level = read<uintptr_t>(gworld + offsets::PersistentLevel);
		if (gworld && level && read<uintptr_t>(level + offsets::OwningWorld) == gworld) {
			break;
		}
	}
...
{
	uint64_t currentactormesh = read<uint64_t>(entity + offsets::ActorMesh);
...
static void FNAMEDecryption(char* inputBuf, int namelength)
{
	...
	v2 = inputBuf;
	v4 = namelength;
	v5 = read<int>(va_text + 0x118D51D4);
	v6 = v5 ^ 0x9C677CC5;
	v7 = v5 % 0x21;
	v8 = 0;
	if (v4)
...
void DrawESP()
{
	...
	Uworld = read<DWORD_PTR>(va_text + offsets::GWORLD);
	...
	for (uint32_t i = 0; i < Num; i++)
	{
		...
		uint64_t CurrentActorMesh = read<uint64_t>(CurrentActor + offsets::ActorMesh);
		...
		if (distance < VisDist)
		{
			...
			if (VisibleCheck) {
				float LastSumbitTime = read<float>(offsets::ActorMesh + offsets::LastSubmitTime);
				float LastRenderTimeOnScreen = read<float>(offsets::ActorMesh + offsets::LastRenderTimeOnScreen);
				bool Visible = LastRenderTimeOnScreen + 0.06f >= LastSumbitTime;
			}
		}
	...
	{ 
		...
		if (spinbot) {
			...
			auto Mesh = read<uint64_t>(LocalPawn + offsets::ActorMesh);
	...
	void render()
	{
		...
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 1.0f, 0.0f, 1.0f)); // Vert
		if (ShowMenu)
		{
			...
			if (ImGui::Button(ICON_FA_BUG "", { 30, 30 }))
			{
				maintabs = 2;
			}
			...
			if (maintabs == 2) {
				ImGui::SetCursorPos({ 80, 20.f });
				ImGui::BeginChild("##child1", ImVec2(240, 240), true);
				ImGui::Text(ICON_FA_GAMEPAD "   Exploits");
				ImGui::Separator();
