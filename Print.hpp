...
{
	SetRed();
	std::cout << E("\nPlease Open Fortnite...");
	SetWhite();
	std::cout << text;
}

void credit3(const char* text)
{
	SetRed();
	std::cout << E("Welcome to Fortnite External\n\n");
	SetWhite();
	std::cout << text;
}
...
void CountDown()
{
	...
	while (num > -1)
	{
		...
		std::cout << E("\r>> ");
		SetWhite();
		std::cout << E("Auto closing in ");

		SetGreen();
		std::cout << num;
		SetWhite();

		std::cout << E(" seconds! Please reload driver...");
		Sleep(1000);
		num--;
	}
}
